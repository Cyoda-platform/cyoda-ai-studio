"""Cyoda AI Assistant using Google ADK Coordinator/Dispatcher Pattern."""
...
# TODO: Enable App pattern when google-adk is updated
...
def create_cyoda_assistant(
    ...
    """Create the Cyoda AI Assistant using Google ADK patterns.

    Uses the Coordinator/Dispatcher pattern with App wrapper and plugins.
    ...
    Returns:
      CyodaAssistantWrapper with the configured agent system
    """
    model_config = _get_model_config()

    # Import get_user_info tool for coordinator to answer environment status questions
    from application.agents.setup.tools import get_user_info

    # Create main coordinator agent with sub-agents
    # sub_agents automatically enables transfer_to_agent tool
    coordinator = LlmAgent(
        ...
        tools=[get_user_info],  # Add get_user_info tool so coordinator can answer environment questions
        sub_agents=[qa_agent, guidelines_agent, setup_agent, environment_agent, github_agent],
    )

    # TODO: Wrap coordinator in App with plugins when google-adk is updated
    # app = App(
    #     name='cyoda_assistant_app',
    ...
    logger.info("✓ Cyoda Assistant created with QA, Guidelines, Setup, Environment, and GitHub sub-agents")
    logger.info("✓ Using sub_agents pattern with LLM-driven delegation")
    logger.info("✓ GitHub Agent handles both complete app builds and incremental changes")

    # Wrap in Cyoda-specific wrapper for session persistence
...
class CyodaAssistantWrapper:
    ...
    def __init__(self, adk_agent: LlmAgent, entity_service: Any):
