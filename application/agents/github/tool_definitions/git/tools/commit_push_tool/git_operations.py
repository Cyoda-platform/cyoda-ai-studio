...
def _is_nothing_to_commit(output: str) -> bool:
    ...
    return (
        GIT_NOTHING_TO_COMMIT_PATTERN1 in output_lower
        or GIT_NOTHING_TO_COMMIT_PATTERN2 in output_lower
    )
...
async def _commit_changes(
    ...
    if commit_process.returncode != 0:
        ...
        # Actual commit error
        logger.error(
            GIT_COMMIT_FAILURE_LOG.format(stdout=stdout_str, stderr=stderr_str)
        )
        error_msg = stderr_str or stdout_str
        return False, GIT_COMMIT_FAILURE_ERROR.format(
            error=error_msg, stop=STOP_ON_ERROR
        )
...
async def _configure_git_and_commit(
    ...
    config = GitConfiguration(
        repository_path=repository_path, commit_message=commit_message
    )
