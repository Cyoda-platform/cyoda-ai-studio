"""Tool for updating application container images."""
...
async def update_application_image(
    ...
    if not env_name or not app_name or not image:
        return json.dumps(
            {"error": "env_name, app_name, and image parameters are required."}
        )

    # 2. Call environment management service
    env_service = get_environment_management_service()
    update_result = await env_service.update_application_image(
        user_id=user_id,
        env_name=env_name,
        app_name=app_name,
        image=image,
