...
class DeployUserApplicationRequest(BaseModel):
    ...
    repository_url: str = Field(..., description="Git repository URL containing the application code")
    ...
    cyoda_client_secret: str = Field(..., description="Cyoda client secret for authentication")
    ...
    is_public: bool = Field(default=True, description="Whether the repository is public")
    installation_id: Optional[str] = Field(default=None, description="GitHub installation ID for public repos")
...
class DeploymentConfig(BaseModel):
    ...
    deployment_type: str = Field(..., description="Type of deployment (e.g., 'environment_deployment')")
    task_name: str = Field(..., description="Name for the background task")
    task_description: str = Field(..., description="Description for the background task")
    env_url: Optional[str] = Field(default=None, description="Optional environment URL")
    additional_metadata: Optional[dict[str, Any]] = Field(
        default=None,
        description="Optional additional metadata to include in task"
    )
...
class SearchLogsRequest(BaseModel):
    ...
    level: Optional[str] = Field(default=None, description="Filter by log level (INFO, ERROR, etc.)")
    start_time: Optional[str] = Field(default=None, description="Start time for log range")
...
class DeployCyodaEnvironmentRequest(BaseModel):
    ...
    env_name: str = Field(..., description="Environment name/namespace to use for deployment")
    build_id: Optional[str] = Field(default=None, description="Optional build ID to associate with deployment")
