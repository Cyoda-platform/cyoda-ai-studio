...
async def get_env_deploy_status(build_id: str) -> str:
    ...
    try:
        ...
        with httpx.Client(timeout=30.0) as client:
            response = client.get(f"{status_url}?build_id={build_id}")
...
async def get_build_context(tool_context: ToolContext) -> str:
    """
    Get build context (language and branch_name) from the conversation's workflow_cache.

    Retrieves the language and branch_name that were stored during the build process.
    ...
    try:
        ...
        if not conversation_id:
            return json.dumps({
                "success": False,
                "error": "No conversation_id found in session state"
            })
        ...
        if language and branch_name:
            logger.info(f"Retrieved build context: language={language}, branch={branch_name}")
            return json.dumps({
                "success": True,
                "language": language,
            ...
            return json.dumps({
                "success": False,
                "error": "No build context found in conversation workflow_cache"
            })
...
async def set_setup_context(
    ...
    try:
        ...
        logger.info(f"Setup context set: {programming_language}, {git_branch}, {repository_name}")
