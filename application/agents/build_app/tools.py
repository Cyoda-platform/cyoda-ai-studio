...
async def check_existing_branch_configuration(
    ...
    try:
        ...
        if repository_name and repository_branch:
            ...
            # Check if repository path exists in context (indicates successful clone)
...
async def _get_authenticated_repo_url_sync(repository_url: str, installation_id: str) -> str:
    """
    Get authenticated repository URL for git operations (synchronous wrapper).
    ...
    async def _get_token() -> str:
        """Inner async function to get the token."""
        token_manager = InstallationTokenManager()
        return await token_manager.get_installation_token(int(installation_id))
    ...
    try:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(asyncio.run, _get_token())
    ...
    **IMPORTANT**: You must call set_repository_config() first to specify whether you want to use:
    ...
    try:
        ...
        if tool_context:
            ...
            logger.info(f"✅ Stored in context: repository_path={target_directory}, branch_name={branch_name}, language={language}, repository_name={repository_name}, repository_owner={repository_owner}")
