...
async def _update_conversation_build_context(
    ...
    def update_fn(conversation: Conversation) -> None:
        ...
        if not owner and conversation.repository_url:
            try:
                ...
                logger.info(
                    f"📦 Extracted repository_owner={owner} from repository_url={conversation.repository_url}"
                )
        ...
        logger.info(
            f"📋 Updated workflow_cache with language={language}, branch_name={branch_name}"
        )
        logger.info(
    ...
    success = await _update_conversation_with_lock(
        conversation_id, update_fn, description="build_context"
    )

    if not success:
        logger.error(
            f"❌ Failed to update conversation build context for {conversation_id}"
        )
...
async def _add_task_to_conversation(conversation_id: str, task_id: str) -> None:
    ...
    """
    logger.info(
        f"🔍 _add_task_to_conversation called: conversation_id={conversation_id}, task_id={task_id}"
    )
    ...
    def update_fn(conversation: Conversation) -> None:
        ...
        if task_id not in conversation.background_task_ids:
            ...
            logger.info(
                f"📋 Added task {task_id} to background_task_ids. Total: {total}"
            )
        else:
            logger.info(f"ℹ️ Task {task_id} already in conversation {conversation_id}")

    success = await _update_conversation_with_lock(
        conversation_id, update_fn, description=f"add_task_{task_id[:8]}"
    )
