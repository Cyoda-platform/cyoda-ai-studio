...
class EnvironmentResourceService:
    ...
    async def get_application_details(self, user_id: str, env_name: str, app_name: str) -> Dict[str, Any]:
        ...
        return await self.deployment_ops.get_application_details(user_id, env_name, app_name)
    ...
    async def scale_application(self, user_id: str, env_name: str, app_name: str, replicas: int) -> Dict[str, Any]:
        ...
        return await self.deployment_ops.scale_application(user_id, env_name, app_name, replicas)

    async def restart_application(self, user_id: str, env_name: str, app_name: str) -> Dict[str, Any]:
        """Restart an application deployment."""
        return await self.deployment_ops.restart_application(user_id, env_name, app_name)
    ...
    async def update_application_image(
        ...
        return await self.deployment_ops.update_application_image(user_id, env_name, app_name, image, container)

    async def get_application_status_rollout(
        self, user_id: str, env_name: str, app_name: str
    ) -> Dict[str, Any]:
        """Get rollout status of an application."""
        return await self.deployment_ops.get_application_status_rollout(user_id, env_name, app_name)
    ...
    async def get_user_app_deployments(self, user_id: str, env_name: str, app_name: str) -> Dict[str, Any]:
        ...
        return await self.user_app_ops.get_user_app_deployments(user_id, env_name, app_name)

    async def scale_user_app(
        self, user_id: str, env_name: str, app_name: str, replicas: int, deployment_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """Scale a user application deployment."""
        return await self.user_app_ops.scale_user_app(user_id, env_name, app_name, replicas, deployment_name)

    async def restart_user_app(
        self, user_id: str, env_name: str, app_name: str, deployment_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """Restart a user application deployment."""
        return await self.user_app_ops.restart_user_app(user_id, env_name, app_name, deployment_name)
    ...
    async def get_user_app_status(
        ...
        return await self.user_app_ops.get_user_app_status(user_id, env_name, app_name, deployment_name)

    async def get_user_app_metrics(self, user_id: str, env_name: str, app_name: str) -> Dict[str, Any]:
        """Get metrics for a user application."""
        return await self.user_app_ops.get_user_app_metrics(user_id, env_name, app_name)

    async def get_user_app_pods(self, user_id: str, env_name: str, app_name: str) -> Dict[str, Any]:
        """Get pods for a user application."""
        return await self.user_app_ops.get_user_app_pods(user_id, env_name, app_name)

    async def delete_user_app(self, user_id: str, env_name: str, app_name: str) -> Dict[str, Any]:
