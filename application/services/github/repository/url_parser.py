...
def parse_repository_url(url: str) -> RepositoryURLInfo:
    ...
    if match:
        ...
        return RepositoryURLInfo(
            owner=owner, repo_name=repo, original_url=url, is_ssh=False
        )
    ...
    if match:
        owner, repo = match.groups()
        return RepositoryURLInfo(
            owner=owner, repo_name=repo, original_url=url, is_ssh=True
        )
...
def construct_repository_url(
    repository_name: str, owner: Optional[str] = None, token: Optional[str] = None
) -> str:
    """
...
def is_custom_repository_url(url: str) -> bool:
    ...
    try:
        ...
        template_repos = [PYTHON_REPOSITORY_NAME, JAVA_REPOSITORY_NAME]
