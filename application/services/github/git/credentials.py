...
class CredentialManager:
    ...
    async def configure_git_user(
        ...
        if user_name:
            ...
            if name_process.returncode != 0:
                ...
                return GitOperationResult(
                    success=False, message="User name config failed", error=error_msg
                )
        ...
        if user_email:
            ...
            if email_process.returncode != 0:
                ...
                logger.error(error_msg)
                return GitOperationResult(
                    success=False, message="User email config failed", error=error_msg
                )
    ...
    async def configure_credential_helper(
        self, helper: str = "store", global_config: bool = True
    ) -> GitOperationResult:
        ...
        if process.returncode != 0:
            ...
            return GitOperationResult(
                success=False,
                message="Credential helper config failed",
                error=error_msg,
            )

        logger.info(f"Git credential helper set to: {helper}")
        return GitOperationResult(
            success=True, message=f"Credential helper set to {helper}"
        )
    ...
    async def store_credentials(
        ...
        try:
            ...
            logger.error(error_msg)
            return GitOperationResult(
                success=False, message="Credential storage failed", error=error_msg
            )

    async def configure_pull_strategy(
        self, rebase: bool = False, global_config: bool = True
    ) -> GitOperationResult:
        ...
        if process.returncode != 0:
            ...
            logger.error(error_msg)
            return GitOperationResult(
                success=False, message="Pull strategy config failed", error=error_msg
            )
    ...
    async def get_git_config(
        self, key: str, global_config: bool = True
    ) -> Optional[str]:
