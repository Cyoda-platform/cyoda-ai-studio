...
class BranchManager:
    ...
    async def create_branch(
        self, clone_dir: str, branch_name: str, base_branch: Optional[str] = None
    ) -> GitOperationResult:
        """Create a new branch.
        ...
        if checkout_base_process.returncode != 0:
            error_msg = (
                f"Error checking out base branch {base_branch}: {stderr.decode()}"
            )
            logger.error(error_msg)
            return GitOperationResult(
                success=False, message="Base checkout failed", error=error_msg
            )
    ...
    async def checkout_branch(
        self, clone_dir: str, branch_name: str
    ) -> GitOperationResult:
        """Checkout a branch.
        ...
        if process.returncode != 0:
            ...
            logger.error(error_msg)
            return GitOperationResult(
                success=False, message="Checkout failed", error=error_msg
            )
    ...
    async def delete_branch(
        self, clone_dir: str, branch_name: str, force: bool = False
    ) -> GitOperationResult:
        """Delete a branch.
        ...
        if process.returncode != 0:
            ...
            logger.error(error_msg)
            return GitOperationResult(
                success=False, message="Branch deletion failed", error=error_msg
            )
    ...
    async def set_upstream(
        self, clone_dir: str, branch_name: str, remote: str = "origin"
    ) -> GitOperationResult:
        ...
        if process.returncode != 0:
            ...
            logger.error(error_msg)
            return GitOperationResult(
                success=False, message="Set upstream failed", error=error_msg
            )

        logger.info(f"Upstream set for branch {branch_name}")
        return GitOperationResult(
            success=True, message=f"Upstream set for {branch_name}"
        )
