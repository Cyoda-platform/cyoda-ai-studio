...
class RepositoryOperations:
    ...
    async def get_repository(
        ...
        if not response:
            raise Exception(
                f"Failed to get repository info for {owner}/{repository_name}"
            )
    ...
    async def list_repositories(
        self, owner: Optional[str] = None, repo_type: str = "all", per_page: int = 30
    ) -> List[Dict[str, Any]]:
    ...
    async def update_repository(
        ...
        response = await self.client.patch(
            f"repos/{owner}/{repository_name}", data=data
        )
    ...
    async def set_repository_topics(
        self, repository_name: str, topics: List[str], owner: Optional[str] = None
    ) -> List[str]:
        ...
        response = await self.client.put(
            f"repos/{owner}/{repository_name}/topics", data={"names": topics}
        )
