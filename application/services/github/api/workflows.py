...
class WorkflowOperations:
    ...
    async def trigger_workflow(
        ...
        dispatch_data = {"ref": ref, "inputs": inputs}
        ...
        run_id = await self._find_run_by_tracker_id(
            owner, repository_name, workflow_id, tracker_id
        )

        if not run_id:
            raise Exception(
                f"Workflow triggered but run not found with tracker_id: {tracker_id}"
            )
    ...
    async def get_workflow_run_status(
        ...
        return WorkflowRunInfo(
            ...
            conclusion=(
                WorkflowConclusion(response["conclusion"])
    ...
    async def monitor_workflow_run(
        ...
        while True:
            run_info = await self.get_workflow_run_status(
                repository_name, run_id, owner
            )

            if run_info.status == WorkflowStatus.COMPLETED:
                logger.info(
                    f"Workflow run {run_id} completed with conclusion: {run_info.conclusion}"
                )
                return run_info
            ...
            if elapsed > timeout_seconds:
                raise Exception(
                    f"Workflow monitoring timed out after {timeout_minutes} minutes"
                )

            logger.info(
                f"Workflow run {run_id} status: {run_info.status}, waiting {poll_interval}s..."
            )
    ...
    async def list_workflow_runs(
        ...
        if workflow_id:
            path = (
                f"repos/{owner}/{repository_name}/actions/workflows/{workflow_id}/runs"
            )
        else:
