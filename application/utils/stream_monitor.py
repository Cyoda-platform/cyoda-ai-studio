...
class StreamMonitor:
    ...
    def start_stream(
        ...
        logger.info(f"Started tracking stream {stream_id} for conversation {conversation_id}")
        return metrics
    ...
    def _update_averages(self):
        ...
        completed_streams = [s for s in self.stream_history if s.state == StreamState.COMPLETED]
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        recent_streams = [s for s in self.stream_history if s.start_time > recent_cutoff]
        recent_failures = [s for s in recent_streams if s.state != StreamState.COMPLETED]
        recent_error_rate = (len(recent_failures) / len(recent_streams) * 100) if recent_streams else 0
    ...
    def cleanup_old_data(self, max_age_hours: int = 24):
        ...
        if len(self.error_counts) > 100:  # Arbitrary limit
            ...
            top_errors = dict(sorted(self.error_counts.items(), key=lambda x: x[1], reverse=True)[:50])
    ...
    def export_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recent_history': [asdict(s) for s in list(self.stream_history)[-100:]],  # Last 100
        }
