...
class GenerateTestTokenRequest(BaseModel):
    ...
    expiry_hours: int = Field(
        default=24, ge=1, le=8760, description="Token expiry in hours"  # Max 1 year
    )
...
class TokenResponse(BaseModel):
    ...
    user_id: str = Field(..., description="User ID in the token")
...
class GuestTokenResponse(BaseModel):
    ...
    guest_id: str = Field(..., description="Guest user ID (starts with 'guest.')")
