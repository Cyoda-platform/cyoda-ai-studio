...
class TestEdgeMessagePersistenceService:
    ...
    @pytest.mark.asyncio
    async def test_save_message_as_edge_message_success(
        self, persistence_service, mock_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_save_message_with_metadata(
        self, persistence_service, mock_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_save_message_with_file_blob_ids(
        self, persistence_service, mock_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_save_streaming_debug_history(
        self, persistence_service, mock_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_save_response_with_history(
        self, persistence_service, mock_repository
    ):
        ...
        call_args = mock_repository.save.call_args
        assert (
            call_args[1]["entity"]["debug_history"]["type"] == "streaming_debug_history"
        )
        assert call_args[1]["entity"]["debug_history"]["events_count"] == 2

    @pytest.mark.asyncio
    async def test_save_response_with_history_and_metadata(
        self, persistence_service, mock_repository
    ):
