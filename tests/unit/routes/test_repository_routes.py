...
class TestAnalyzeRepository:
    ...
    async def test_analyze_repository_error(self, client):
        ...
        with patch(
            "application.routes.repository_endpoints.analyze.legacy_analysis.RepositoryParser"
        ) as mock_parser_class:
            with patch(
                "application.routes.repository_endpoints.analyze.legacy_analysis.get_github_service_for_public_repo"
            ):
                mock_parser = MagicMock()
                mock_parser_class.return_value = mock_parser
                mock_parser.parse_repository = AsyncMock(
                    side_effect=Exception("Parse error")
                )

                response = await client.post(
                    "/api/v1/repository/analyze",
                    json={"repository_name": "test-repo", "branch": "main"},
                )
    ...
    async def test_analyze_repository_with_requirements(self, client):
        ...
        with patch(
            ...
            with patch(
                ...
                mock_github.contents.get_file_content = AsyncMock(
    ...
    async def test_analyze_repository_conversation_not_found(self, client):
        ...
        with patch(
            "application.routes.repository_endpoints.analyze.get_entity_service"
        ) as mock_service:
            ...
            response = await client.post(
                "/api/v1/repository/analyze", json={"conversation_id": "nonexistent"}
            )
    ...
    async def test_analyze_repository_conversation_dict_format(self, client):
        ...
        with patch(
            ...
            mock_response.data = {
                ...
                "workflow_cache": {
                    "adk_session_state": {"repository_path": "/tmp/repo"}
                },
            }
            ...
            with patch(
                "application.agents.github.tools._detect_project_type"
            ) as mock_detect:
                ...
                with patch(
                    "application.agents.github.tools._scan_versioned_resources"
                ) as mock_scan:
...
class TestGetFileContent:
    ...
    async def test_get_file_content_success(self, client):
        ...
        with patch(
            "application.routes.repository_endpoints.file_content.get_github_service_for_public_repo"
        ) as mock_service:
            mock_github = MagicMock()
            mock_github.contents.get_file_content = AsyncMock(
    ...
    async def test_get_file_content_missing_fields(self, client):
        ...
        response = await client.post(
            "/api/v1/repository/file-content", json={"repository_name": "test-repo"}
        )
    ...
    async def test_get_file_content_not_found(self, client):
        ...
        with patch(
            ...
            response = await client.post(
                "/api/v1/repository/file-content",
                json={"repository_name": "test-repo", "file_path": "nonexistent.py"},
            )
            assert response.status_code == 404
    ...
    async def test_get_file_content_error(self, client):
        ...
        with patch(
            ...
            response = await client.post(
                "/api/v1/repository/file-content",
                json={"repository_name": "test-repo", "file_path": "test.py"},
            )
...
class TestGetRepositoryDiff:
    ...
    async def test_get_diff_success(self, client):
        ...
        with patch(
            "application.routes.repository_endpoints.diff.subprocess.run"
        ) as mock_run:
            ...
            response = await client.post(
                "/api/v1/repository/diff", json={"repository_path": "/tmp/repo"}
            )
            assert response.status_code == 200
            data = await response.get_json()
    ...
    async def test_get_diff_missing_path(self, client):
        ...
        response = await client.post("/api/v1/repository/diff", json={})
        assert response.status_code == 400
...
class TestPullRepository:
    ...
    async def test_pull_repository_missing_conversation_id(self, client):
        ...
        response = await client.post("/api/v1/repository/pull", json={})
        assert response.status_code == 400
    ...
    async def test_pull_repository_conversation_not_found(self, client):
        ...
        with patch(
            "application.routes.repository_endpoints.pull.get_entity_service"
        ) as mock_service:
            mock_entity_service = MagicMock()
            mock_entity_service.get_by_id = AsyncMock(return_value=None)
            mock_service.return_value = mock_entity_service
            ...
            response = await client.post(
                "/api/v1/repository/pull", json={"conversation_id": "nonexistent"}
            )
            assert response.status_code == 404
    ...
    async def test_pull_repository_no_branch_configured(self, client):
        ...
        with patch(
            ...
            response = await client.post(
                "/api/v1/repository/pull", json={"conversation_id": "conv_123"}
            )
            assert response.status_code == 400
...
class TestEnsureRepositoryCloned:
    ...
    async def test_ensure_repository_cloned_already_exists(self):
        ...
        with patch(
            "application.routes.repository_endpoints.helpers._is_repository_already_cloned"
        ) as mock_is_cloned:
            mock_is_cloned.return_value = True

            success, message, path = await ensure_repository_cloned(
                repository_url="https://github.com/test/repo", repository_branch="main"
            )
    ...
    async def test_ensure_repository_cloned_invalid_url(self):
        ...
        success, message, path = await ensure_repository_cloned(
            repository_url="invalid-url", repository_branch="main"
        )
    ...
    async def test_ensure_repository_cloned_timeout(self):
        ...
        with patch(
            "application.routes.repository_endpoints.helpers.subprocess.run"
        ) as mock_run:
            mock_run.side_effect = Exception("Timeout")

            with patch(
                "application.routes.repository_endpoints.helpers.Path"
            ) as mock_path_class:
                mock_path = MagicMock()
    ...
    async def test_ensure_repository_cloned_with_installation_id(self):
        ...
        with patch(
            "application.routes.repository_endpoints.helpers.InstallationTokenManager"
        ) as mock_token_mgr:
            mock_mgr = MagicMock()
