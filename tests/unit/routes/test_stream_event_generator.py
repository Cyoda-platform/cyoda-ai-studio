...
class TestCreateStreamEventGenerator:
    ...
    async def test_event_generator_with_content_events(self):
        ...
        with patch("application.routes.chat_endpoints.stream.StreamingService.stream_agent_response", return_value=mock_stream()):
            with patch("application.routes.chat_endpoints.stream.process_content_event") as mock_process_content:
                with patch("application.routes.chat_endpoints.stream.process_done_event") as mock_process_done:
                    with patch("application.routes.chat_endpoints.stream._finalize_stream", new_callable=AsyncMock):
                        mock_process_content.return_value = ("Hello", [])
                        mock_process_done.return_value = (True, 'event: done\ndata: {}\n\n', "Hello", None, None, [])
    ...
    async def test_event_generator_finalizes_on_completion(self):
        ...
        with patch("application.routes.chat_endpoints.stream.StreamingService.stream_agent_response", return_value=mock_stream()):
            with patch("application.routes.chat_endpoints.stream.process_done_event") as mock_process_done:
                with patch("application.routes.chat_endpoints.stream._finalize_stream", new_callable=AsyncMock) as mock_finalize:
                    mock_process_done.return_value = (True, 'event: done\ndata: {}\n\n', "Done", None, None, [])
