...
from application.services.repository_parser import EntityInfo, RepositoryParser
...
class TestParsePythonEntities:
    ...
    async def test_parse_python_entities_success(self):
        ...
        github_service.contents.list_directory = AsyncMock(
            side_effect=[[mock_entity_item], [mock_version_item]]
        )

        github_service.contents.file_exists = AsyncMock(return_value=True)
        github_service.contents.get_file_content = AsyncMock(
            return_value="class User:\n    pass"
        )
        ...
        parser._extract_fields = MagicMock(
            return_value=[{"name": "id", "type": "string"}]
        )
    ...
    async def test_parse_python_entities_skip_private_directories(self):
        ...
        github_service.contents.list_directory = AsyncMock(
            return_value=[mock_private_item]
        )
    ...
    async def test_parse_python_entities_skip_invalid_version_format(self):
        ...
        github_service.contents.list_directory = AsyncMock(
            side_effect=[[mock_entity_item], [mock_invalid_version]]
        )
    ...
    async def test_parse_python_entities_multiple_versions(self):
        ...
        github_service.contents.list_directory = AsyncMock(
            side_effect=[[mock_entity_item], [mock_version1, mock_version2]]
        )
