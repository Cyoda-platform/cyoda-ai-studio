    ...
    async def test_ensure_repository_extract_name_from_url(self):
        ...
        with patch("pathlib.Path.exists", return_value=False):
            with patch("pathlib.Path.mkdir"):
                ...
                with patch(
                    "application.services.github.operations_service.service.repository_operations.run_subprocess"
                ) as mock_run:
        ...
        with patch("pathlib.Path.exists", return_value=False):
            with patch("pathlib.Path.mkdir"):
                with patch(
                    ...
                    with patch(
                        "application.services.github.operations_service.InstallationTokenManager"
                    ) as mock_token_mgr:
                        mock_mgr_instance = AsyncMock()
                        mock_mgr_instance.get_installation_token = AsyncMock(
