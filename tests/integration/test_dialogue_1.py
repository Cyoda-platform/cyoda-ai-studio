...
from application.agents.cyoda_assistant.wrapper import (  # Import the class directly
...
async def test_initial_business_problem_routing(mocker):
    ...
    mocker.patch(
        "application.agents.cyoda_assistant.wrapper.CyodaAssistantWrapper.process_message",
        new=mock_process_message,
    )
    ...
    assert (
        "I'll help you build a solution that enables an institutional trading platform..."
        in result["response"]
    )
