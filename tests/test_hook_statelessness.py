...
class TestHookStatelessness:
    ...
    @pytest.mark.asyncio
    async def test_new_hook_sent_in_done_event(self, mock_agent_wrapper, mock_session):
        """Test that new hooks created in stream are sent in done event."""
        # Setup: Session starts with no hook
        mock_session.state = {}
        ...
        # Mock the runner to create a new hook
        async def mock_run_async(*args, **kwargs):
            # Simulate tool creating a hook
            mock_session.state["last_tool_hook"] = new_hook
            yield MagicMock(parts=[])
        ...
        # Consume the stream and find done event
        done_event = None
        async for event_str in stream:
            if "done" in event_str and "hook" in event_str:
                done_event = event_str
                break

        # Verify: Done event contains the new hook
        assert done_event is not None, "Done event should be sent"
        assert "option_selection" in done_event, "Done event should contain the new hook"
